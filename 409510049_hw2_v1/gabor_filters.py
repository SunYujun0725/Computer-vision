...
def extract_gabor_feature(img_path):
    img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)  # 讀取圖像並轉換為灰度
    img = cv2.resize(img, (256, 256))
    # 計算Gabor特徵
    theta = 0  # Gabor filter 的角度
    frequency = 0.6  # Gabor filter 的頻率
    gabor_result, _ = gabor(img, frequency=frequency, theta=theta)
    gabor_result = gabor_result.flatten()
    hist, bins = np.histogram(gabor_result, bins=256, range=[0, 256])
    return hist
    ...
    if is_train: #有兩層資料夾要處理
        ...
        for label in os.listdir(folder_path):
            ...
            for filename in os.listdir(label_folder):
                ...
                # 提取特徵（顏色直方圖）
                hist = extract_gabor_feature(img_path)
                # 將特徵和標籤添加到 DataFrame
...
def main():
    ...
    # 將結果保存為 CSV 檔
    results_sad.to_csv('gabor_filters_sad.csv', index=False)
